...
export const StudentCoordinatorsSection: React.FC = () => {
  const openDetailPage = () => window.open('/student-coordinators.html', '_blank');
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <ScrollReveal animation="up">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-border">
            <div>
              <div className="inline-block text-xs font-bold tracking-[0.25em] text-phoenix-orange uppercase mb-3">06 / STUDENT COORDINATORS</div>
              <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold tracking-tight text-foreground">STUDENT COORDINATORS</h2>
            </div>
            <div className="mt-4 md:mt-0 flex items-center gap-3">
              ...
              <button onClick={openDetailPage} className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-white border border-border hover:border-phoenix-orange hover:text-phoenix-orange text-xs font-bold tracking-wider uppercase text-foreground-secondary transition-all duration-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {STUDENT_COORDINATORS.map((student, idx) => (
            <ScrollReveal key={student.name} animation="up" delay={idx * 70}>
              <div onClick={openDetailPage} className="group p-7 rounded-2xl bg-white border border-border hover:border-phoenix-orange transition-all duration-300 shadow-sm hover:shadow-phoenix-subtle flex flex-col justify-between cursor-pointer h-full">
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full bg-background-warm text-phoenix-red border border-border">{student.role}</span>
                    {student.phone
                      ? <Phone className="w-3.5 h-3.5 text-phoenix-orange" />
                      : <div className="w-2 h-2 rounded-full bg-phoenix-orange/40 group-hover:bg-phoenix-orange transition-colors" />}
                  </div>
                  <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground mb-1 group-hover:text-darkAccent">{student.name}</h3>
                  {student.year && (
                    <p className="text-xs font-bold tracking-wider uppercase text-phoenix-magenta mb-1">{student.year}</p>
                  )}
        ...
        <ScrollReveal animation="fade" delay={200}>
          <div className="mt-12 flex justify-center">
            <button onClick={openDetailPage} className="phoenix-gradient-btn px-8 py-3.5 rounded-full text-white text-xs sm:text-sm font-bold tracking-wider uppercase flex items-center gap-2 shadow-sm">
              <ExternalLink className="w-4 h-4" /> OPEN FULL STUDENT COORDINATORS PAGE
            </button>
          </div>
        </ScrollReveal>
      </div>
