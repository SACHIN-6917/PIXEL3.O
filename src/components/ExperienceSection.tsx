  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'UtensilsCrossed':
        return <UtensilsCrossed className="w-6 h-6 text-foreground group-hover:text-phoenix-orange transition-colors" />;
      case 'Coffee':
        return <Coffee className="w-6 h-6 text-foreground group-hover:text-phoenix-red transition-colors" />;
      case 'Trophy':
        return <Trophy className="w-6 h-6 text-foreground group-hover:text-phoenix-gold transition-colors" />;
      case 'Award':
      default:
        return <Award className="w-6 h-6 text-foreground group-hover:text-phoenix-magenta transition-colors" />;
    }
  };
  ...
  return (
    <section className="py-24 bg-background-warm border-t border-border relative overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* 4 Premium Experience Blocks */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {EXPERIENCE_PERKS.map((perk) => (
            <div
              key={perk.title}
              className="group p-7 rounded-2xl bg-white border border-border hover:border-phoenix-orange/40 transition-all duration-300 shadow-sm hover:shadow-phoenix-subtle flex flex-col justify-between"
            >
              <div>
                ...
                <h3 className="text-lg font-bold tracking-tight text-foreground mb-2 group-hover:text-darkAccent">
                  {perk.title}
                </h3>
