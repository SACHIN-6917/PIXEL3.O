...
export const StaffCoordinatorsSection: React.FC = () => {
  const openDetailPage = () => {
    window.open('/staff-coordinators.html', '_blank');
  };
  ...
  return (
    <section id="staff" className="py-24 bg-white border-t border-border relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 pb-6 border-b border-border">
          ...
          <div className="mt-4 md:mt-0 flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-background-warm border border-border hover:border-phoenix-orange hover:text-phoenix-orange text-xs font-bold tracking-wider uppercase text-foreground-secondary transition-all duration-200"
            >
        ...
        {/* 6 Staff Coordinators Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {STAFF_COORDINATORS.map((staff) => (
            <div
              key={staff.name}
              onClick={openDetailPage}
              className="group p-7 rounded-2xl bg-white border border-border hover:border-phoenix-orange transition-all duration-300 shadow-sm hover:shadow-phoenix-subtle relative flex flex-col justify-between cursor-pointer"
            >
        ...
        <div className="mt-12 flex justify-center">
          ...
          >
            ...
            OPEN FULL STAFF COORDINATORS PAGE
          </button>
        </div>
      </div>
    </section>
