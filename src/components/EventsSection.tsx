...
export const EventsSection: React.FC<EventsSectionProps> = ({ onRegisterEvent }) => {
  ...
  return (
    <section id="events" className="py-24 bg-white border-t border-border relative">
          ...
          {CONFIRMED_EVENTS.map((event) => {
            ...
            return (
              <div
                ...
                className={`group cursor-pointer relative py-8 sm:py-10 px-4 sm:px-6 transition-all duration-300 ${
                  isHovered ? 'bg-background-warm/60 -translate-y-0.5' : 'bg-white'
                }`}
              >
                {/* Traveling Gradient Line on Hover */}
                <div
                  className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-phoenix-orange via-phoenix-red to-phoenix-magenta transition-opacity duration-300 ${
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <div className="flex items-start sm:items-center gap-6 sm:gap-10">
                    <span
                      className={`text-2xl sm:text-3xl md:text-4xl font-mono font-bold tracking-tight transition-colors duration-300 ${
                        isHovered ? 'text-phoenix-orange' : 'text-foreground-muted'
                      }`}
                    >
                      {event.number}
                    </span>
                    ...
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-foreground-muted line-clamp-1 max-w-xl">
                        {event.description}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between md:justify-end gap-6 sm:gap-8 pl-12 sm:pl-0">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold tracking-wider text-phoenix-orange uppercase hidden sm:inline-block">
                        VIEW RULES
                      </span>
      ...
      {/* Event Details Modal */}
      {selectedEvent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-darkAccent/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl border border-border relative max-h-[90vh] overflow-y-auto">
            ...
            <button
              ...
              className="absolute top-6 right-6 p-2 rounded-full hover:bg-background-secondary text-foreground-secondary hover:text-darkAccent transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-3 mb-2">
              <span className="text-sm font-mono font-bold text-phoenix-orange">
                EVENT {selectedEvent.number}
              </span>
              <span className="text-xs px-2.5 py-0.5 font-bold uppercase rounded-full bg-background-warm border border-border text-foreground-secondary">
            ...
            {/* Description */}
            <p className="text-sm sm:text-base text-foreground-secondary mb-6 leading-relaxed">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-background-warm/70 border border-border mb-6 text-xs sm:text-sm">
              <div className="flex items-center gap-2.5">
                <MapPin className="w-4 h-4 text-phoenix-red flex-shrink-0" />
                <div>
                  <div className="text-[10px] text-foreground-muted uppercase font-bold">VENUE</div>
                  <div className="font-semibold text-foreground">{selectedEvent.venue}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[10px] text-foreground-muted uppercase font-bold">TIMING</div>
                  <div className="font-semibold text-foreground">{selectedEvent.timing}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-[10px] text-foreground-muted uppercase font-bold">TEAM SIZE</div>
                  <div className="font-semibold text-foreground">{selectedEvent.teamSize}</div>
                </div>
              </div>
            </div>
            ...
            {/* Guidelines / Rules */}
            <div className="mb-8">
              <h4 className="text-sm font-bold tracking-wider uppercase text-foreground mb-3">
                RULES & FORMAT
              </h4>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-border">
              <button
                onClick={() => setSelectedEvent(null)}
                className="w-full sm:w-auto px-6 py-3 rounded-full border border-border text-foreground-secondary hover:text-darkAccent text-xs font-bold uppercase tracking-wider transition-colors"
              >
                CLOSE
              </button>
              ...
              <button
                ...
                className="w-full sm:w-auto phoenix-gradient-btn px-8 py-3 rounded-full text-white text-xs font-bold uppercase tracking-wider shadow-sm flex items-center justify-center gap-2"
              >
