...
export const AboutSection: React.FC = () => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'Cpu': return <Cpu className="w-6 h-6 text-foreground group-hover:text-phoenix-orange transition-colors" />;
      case 'Lightbulb': return <Lightbulb className="w-6 h-6 text-foreground group-hover:text-phoenix-red transition-colors" />;
      case 'Trophy': return <Trophy className="w-6 h-6 text-foreground group-hover:text-phoenix-gold transition-colors" />;
      case 'Sparkles': default: return <Sparkles className="w-6 h-6 text-foreground group-hover:text-phoenix-magenta transition-colors" />;
    }
  };

  return (
    <section id="about" className="py-24 bg-white relative overflow-hidden">
      {/* Subtle abstract Phoenix feather backdrop */}
      <div className="absolute top-1/2 right-[-10%] -translate-y-1/2 w-[600px] h-[600px] rounded-full blur-[140px] opacity-15 pointer-events-none"
        style={{ background: 'radial-gradient(circle, #FF6A00 0%, #E0008A 45%, #6A00FF 90%)' }} />
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <ScrollReveal animation="fade" delay={80}>
          <div className="flex items-center gap-3 mb-8">
            <h3 className="text-xl sm:text-2xl font-bold tracking-tight text-foreground">WHAT AWAITS YOU</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {WHAT_AWAITS_YOU.map((item, idx) => (
            <ScrollReveal key={item.title} animation="up" delay={idx * 80}>
              <div className="group p-6 rounded-2xl bg-background-warm/60 border border-border hover:border-phoenix-orange/40 hover:bg-white transition-all duration-300 shadow-sm hover:shadow-phoenix-subtle flex flex-col justify-between h-full">
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-foreground-secondary leading-relaxed">{item.description}</p>
                </div>
