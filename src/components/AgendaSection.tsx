              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-6 md:left-1/2 top-6 -translate-x-1/2 z-10 flex items-center justify-center">
                    <div
                      className={`w-6 h-6 rounded-full border-2 bg-white flex items-center justify-center transition-all duration-300 ${
                        item.highlight
                          ? 'border-phoenix-orange shadow-[0_0_12px_rgba(255,106,0,0.4)]'
                          : 'border-border group-hover:border-phoenix-red'
                      }`}
                    >
                      <div
                        className={`w-2.5 h-2.5 rounded-full transition-colors ${
                          item.highlight
                            ? 'bg-gradient-to-r from-phoenix-orange to-phoenix-gold'
                            : 'bg-foreground-muted group-hover:bg-phoenix-red'
                        }`}
                      />
                    </div>
                  </div>

                  {/* Card Content (Offset for left/right on desktop) */}
                  <div className="ml-14 md:ml-0 md:w-1/2 md:px-8 w-full">
                    <div
                      className={`p-6 sm:p-7 rounded-2xl bg-white border border-border transition-all duration-300 shadow-sm hover:shadow-phoenix-subtle ${
                        item.highlight
                          ? 'border-phoenix-orange/40 bg-gradient-to-br from-white via-white to-background-warm'
                          : 'hover:border-phoenix-orange/30'
                      }`}
                    >
                      {/* Badge & Timing */}
                      ...
                      {/* Subtitle */}
                      {item.subtitle && (
                        <p className="text-xs sm:text-sm font-semibold text-phoenix-red tracking-wide mb-2">
                          {item.subtitle}
                        </p>
                      )}
